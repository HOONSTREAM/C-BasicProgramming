...
while (++first != last ) {
  ...
  if(!(*result==*first))     //result 값과 first 값이 다르면 (혹은 조건자 버전일 경우 조건자 함수가 참이 아니면)
     *(++result)=*first     //result 칸을 먼저 증가 시키고 first 값으로 채운다. if문이 false면  while문으로 되돌아가서 first 칸만 증가시킨다.
}
...
bool Pred(int left, int right) {
    ...
    return abs(right - left) < 10; //인접 원소의 차가 10이하인 것이 참이다.

}

//조건자 함수가 참이 아니면 while문 동작
...
int main(void) {
   ...
   cout << "조건자 unique 알고리즘 실행 후 벡터 v : ";
   for (auto& elem : v)
       cout << elem << " ";
   cout << endl;
